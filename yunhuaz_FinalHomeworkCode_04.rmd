...
plot1 <- ggplot(data, aes(x = Brain_Size_Species_Mean, y = MaxLongevity_m)) +
  ...
  geom_text(
    aes(x = max(Brain_Size_Species_Mean) * 0.8, y = max(MaxLongevity_m) * 0.9),
    label = paste("y =", round(coef(m1)[1], 2), "+", round(coef(m1)[2], 2), "x"),
    color = "blue"
  ) +
  labs(
    title = "Longevity ~ Brain Size",
    x = "Brain Size (g)",
    y = "Longevity (months)"
  )
...
plot2 <- ggplot(data, aes(x = log(Brain_Size_Species_Mean), y = log(MaxLongevity_m))) +
  ...
  geom_text(
    aes(x = max(log(Brain_Size_Species_Mean)) * 0.8, y = max(log(MaxLongevity_m)) * 0.9),
    label = paste("y =", round(coef(m2)[1], 2), "+", round(coef(m2)[2], 2), "x"),
    color = "purple"  
  ) +
  labs(
    title = "Log(Longevity) ~ Log(Brain Size)",
    x = "Log(Brain Size)",
    y = "Log(Longevity)"
  )
